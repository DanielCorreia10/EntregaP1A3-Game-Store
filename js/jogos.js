...
function toggleCart(gameId) {
  ...
  if (idx === -1) { cart.push(gameId); showToast('Adicionado ao carrinho!', 'success'); }
  else { cart.splice(idx, 1); showToast('Removido do carrinho.'); }
  updateCartCount();
  renderGames();
}
...
function toggleWishlist(gameId) {
  ...
  if (idx === -1) { wishlist.push(gameId); showToast('Adicionado à lista de desejos!'); }
...
function makeCardSvg(colors) {
  ...
  return `<svg class="card-gradient" viewBox="0 0 300 170" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
    <defs>
      <radialGradient id="rg_${id}" cx="30%" cy="30%" r="70%">
        <stop offset="0%" stop-color="${c1}"/>
        <stop offset="100%" stop-color="${c2}"/>
      </radialGradient>
    </defs>
    <rect width="300" height="170" fill="url(#rg_${id})"/>
...
function renderGames() {
  ...
  if (activeGenre !== 'Todos') {
    filtered = filtered.filter(g => g.genre.toLowerCase() === activeGenre.toLowerCase());
  }
  ...
  gamesGridEl.innerHTML = filtered.map(game => {
    ...
    <article class="game-card" data-id="${game.id}">
      <div class="card-image">
        ...
        <button class="card-wishlist-float ${inWish ? 'active' : ''}" data-wish="${game.id}">
      ...
      <div class="card-body">
        ...
        <div class="card-actions">
          ...
          <button class="btn-wishlist ${inWish ? 'active' : ''}" data-wish="${game.id}">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="${inWish ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="2.2">
              <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
            </svg>
          </button>
          <button class="btn-cart ${inCart ? 'in-cart' : ''}" data-cart="${game.id}">
  ...
  gamesGridEl.querySelectorAll('[data-cart]').forEach(btn => {
    btn.addEventListener('click', e => { e.stopPropagation(); toggleCart(Number(btn.dataset.cart)); });
  });

  gamesGridEl.querySelectorAll('[data-wish]').forEach(btn => {
    btn.addEventListener('click', e => { e.stopPropagation(); toggleWishlist(Number(btn.dataset.wish)); });
  });
}

searchInput.addEventListener('input', () => { searchQuery = searchInput.value; renderGames(); });
