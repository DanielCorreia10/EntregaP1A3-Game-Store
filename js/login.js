...
function handleLogin() {
  ...
  if (!email) { showToast('Informe seu e-mail.', 'error'); emailInput.focus(); return; }
  if (!email.includes('@')) { showToast('E-mail inválido.', 'error'); emailInput.focus(); return; }
  if (!senha) { showToast('Informe sua senha.', 'error'); senhaInput.focus(); return; }
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { window.location.href = 'jogos.html'; }, 800);
  }, 1000);
}